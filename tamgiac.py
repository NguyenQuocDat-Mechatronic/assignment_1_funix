...
def kiemtra_tamgiac(input):
    ...
    """cần dùng round để chủ động làm tròn chứ ko để python tự làm tròn gây lỗi case [1,1,2,2,4,4]"""
    # AB = round(sqrt((vecto_AB[0]) ** 2 + (vecto_AB[1]) ** 2), 2)
    # BC = round(sqrt((vecto_BC[0]) ** 2 + (vecto_BC[1]) ** 2), 2)
    # CA = round(sqrt((vecto_CA[0]) ** 2 + (vecto_CA[1]) ** 2), 2)
...
if kiemtra_tamgiac(input) is True :
    ...
    def duongcao_tamgiac(input):
        ...
        """ inputng thuc heron ha = 2*sqrt(p*(p - a) * (p - b) * (p - c))/a"""
        p = (AB + BC + CA) / 2
